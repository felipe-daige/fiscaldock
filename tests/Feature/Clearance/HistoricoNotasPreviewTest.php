...
it('exibe preview expansivel dos resultados de cada lote no historico de notas', function () {
    ...
    actingAs($user)
        ...
        ->assertSee('data-history-result-url="'.route('app.clearance.notas.resultado', ['consultaLoteId' => $lote->id]).'"', false)
        ->assertSee('Ver detalhes')
        ->assertSee('data-history-details="lote-'.$lote->id.'"', false)
        ->assertSee('Verificação realizada')
        ->assertSee('Fornecedor Cancelado → Cliente do Histórico')
        ->assertSee('+ 1 documento')
        ->assertSee('Divergências críticas')
        ->assertSee('1 autorizada · 1 alerta · 0 a revisar')
