{{-- Dossiê do CLIENTE. Reusa os partials do dossiê do participante sem editá-los: o cliente é
     passado no slot `participante` dos includes (Cliente expõe razao_social/situacao_cadastral/
     documento/uf, lidos pelos partials). $score/$movimentacao/$consulta vêm do payload e ficam
     no escopo da view, herdados pelos includes. --}}
...
@section('conteudo')
    @include('reports.dossie._resumo', ['participante' => $cliente])
    @include('reports.dossie._infograficos', ['participante' => $cliente])
    @include('reports.dossie._listas-movimentacao', ['top_produtos' => $top_produtos ?? [], 'top_cfops' => $top_cfops ?? []])
    @include('reports.dossie._detalhamento', ['participante' => $cliente])
@endsection
