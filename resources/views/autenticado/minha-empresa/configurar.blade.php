            ...
            @if(($clientes ?? collect())->count() > 0)
                <div class="p-4 space-y-2" id="lista-empresas">
                    @foreach($clientes as $cliente)
                        ...
                        <div class="empresa-item flex items-center justify-between p-4 rounded border transition-all {{ $ativa ? 'border-gray-800 bg-gray-50' : 'border-gray-300 cursor-pointer hover:bg-gray-50/50' }}"
                             data-cliente-id="{{ $cliente->id }}"
                             @if(! $ativa)
                                 role="button"
                                 tabindex="0"
                                 onclick="definirPrincipal({{ $cliente->id }}, this)"
                                 onkeydown="if (event.target === this && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); definirPrincipal({{ $cliente->id }}, this); }"
                             @endif>
                            ...
                            <div class="flex items-center gap-3">
                                @if($ativa)
                                    <span class="w-28 whitespace-nowrap inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded text-xs font-semibold text-white"
                                ...
                                @else
                                    <button type="button" class="btn-selecionar w-28 inline-flex items-center justify-center px-3 py-1.5 rounded bg-gray-800 hover:bg-gray-700 text-white text-xs font-semibold transition" onclick="event.stopPropagation(); definirPrincipal({{ $cliente->id }}, this)">
                                        Selecionar
                                    </button>
                                @endif
                            </div>
...
<script>
function destacarEmpresa(clienteId) {
...
function definirPrincipal(clienteId, triggerElement) {
    const selectedItem = triggerElement?.closest('.empresa-item')
        || document.querySelector(`[data-cliente-id="${clienteId}"]`);

    if (!selectedItem || selectedItem.dataset.salvando === 'true') {
        return;
    }

    destacarEmpresa(clienteId);
    selectedItem.dataset.salvando = 'true';
    selectedItem.setAttribute('aria-busy', 'true');

    const btn = selectedItem.querySelector('.btn-selecionar');
    ...
    const restaurarItem = () => {
        delete selectedItem.dataset.salvando;
        selectedItem.removeAttribute('aria-busy');
