...
@php
    ...
    foreach ($planos->where('is_active', true) as $p) {
        ...
        $planosDetalhados[] = [
            ...
            // "em breve" aposentado: todas as fontes dos planos ativos estão LIVE (FonteRegistry::cobre)
            'consultas_em_breve' => [],
            'consultas_quebra_antes' => [],
                                ...
                                <td class="px-3 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($plano['popular'] && ! $plano['coming_soon'])
                                            <span class="whitespace-nowrap px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide text-white" style="background-color: #374151">Popular</span>
                                        @endif
                                    </div>
                                    @if($plano['descricao'])
