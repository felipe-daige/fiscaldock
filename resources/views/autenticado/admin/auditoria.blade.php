            ...
            <table class="w-full text-sm tabela-cards">
                ...
                @forelse($logs as $log)
                    <tr>
                        ...
                        <td class="px-3 py-2" data-label="Operador">{{ $log->admin->name ?? '—' }}</td>
                        <td class="px-3 py-2" data-label="Alvo">{{ $log->alvo->name ?? '—' }}</td>
                        <td class="px-3 py-2" data-label="Ação"><span class="text-[11px] font-semibold text-gray-700">{{ $log->acao }}</span></td>
