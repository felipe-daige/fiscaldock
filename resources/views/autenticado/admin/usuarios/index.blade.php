...
<div class="min-h-screen bg-gray-100">
    <div class="admin-page max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <form method="GET" class="bg-white rounded border border-gray-300 p-3 mb-4 flex flex-col sm:flex-row sm:items-end gap-3">
            <div class="flex-1">
            ...
            <table class="w-full text-sm tabela-cards">
                <thead class="bg-gray-50 text-[10px] uppercase tracking-wide text-gray-400">
                    <tr>
                        ...
                        <th class="text-right px-3 py-2.5 w-44">Ações</th>
                    </tr>
                </thead>
                ...
                @forelse($usuarios as $u)
                    ...
                    <tr class="hover:bg-gray-50/60">
                        {{-- Usuário --}}
                        <td class="px-3 py-2.5" data-label="Usuário">
                            <div class="flex items-start gap-2">
                                <span class="text-[10px] font-mono text-gray-300 pt-0.5">#{{ $u->id }}</span>
                                <div class="min-w-0">
                                    <button type="button" onclick="document.getElementById('admin-user-modal-{{ $u->id }}').showModal()" class="text-left font-medium text-gray-900 hover:text-blue-700 hover:underline cursor-pointer">{{ $u->name }} {{ $u->sobrenome }}</button>
                        ...
                        {{-- Empresa --}}
                        <td class="px-3 py-2.5 text-gray-700" data-label="Empresa">
                        ...
                        <td class="px-3 py-2.5" data-label="Plano">
                            <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase text-white" style="background-color:{{ $planoCor }}">{{ $plano }}</span>
                            @if($u->assinatura_status && $u->assinatura_status !== 'ativa')
                                <span class="block text-[10px] text-gray-400 mt-0.5">{{ $u->assinatura_status }}</span>
                            @endif
                        </td>
                        {{-- Saldo --}}
                        <td class="px-3 py-2.5 text-right" data-label="Saldo">
                            <span class="block font-semibold text-gray-900">{{ $fmtN($u->credits) }}</span>
                            <span class="block text-[10px] text-gray-400">≈ {{ $emReais($u->credits) }}</span>
                        </td>
                        {{-- Atividade --}}
                        <td class="px-3 py-2.5 text-[12px] text-gray-600" data-label="Atividade">
                        ...
                        {{-- Uso --}}
                        <td class="px-3 py-2.5 text-right text-[12px] text-gray-600" data-label="Uso">
                        ...
                        <td class="px-3 py-2.5 text-right" data-label="Ações">
                            <div class="flex flex-col sm:flex-row justify-end gap-1.5">
                                <button type="button" onclick="document.getElementById('admin-user-modal-{{ $u->id }}').showModal()" class="inline-flex items-center justify-center px-3 py-1.5 rounded text-[11px] font-bold uppercase tracking-wide text-white hover:opacity-90" style="background-color:#0b1f3a">Plano/saldo</button>
                                <a href="{{ route('app.admin.usuarios.edit', $u->id) }}" data-link class="admin-action inline-flex items-center justify-center px-3 py-1.5 rounded border border-gray-300 text-[11px] font-bold uppercase tracking-wide text-gray-700 bg-white hover:bg-gray-50">Cadastro</a>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="px-3 py-10 text-center text-gray-400 text-sm">
        ...
        @foreach($usuarios as $u)
            @php
                ...
                $modalBucket = (int) old('creditos_inclusos_saldo', $u->assinatura_bucket ?? 0);
            ...
            <dialog
                ...
                style="width:min(94vw, 1000px); height:88vh; max-height:88vh; overflow:hidden"
            >
                <div class="bg-white relative" data-admin-action-scope style="display:flex; flex-direction:column; height:100%; width:100%">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <select name="subscription_plan_id" class="w-full text-[13px] py-2.5 px-3 border border-gray-300 rounded">
                                    <option value="">Sem assinatura local (Free)</option>
                            ...
                            <div class="grid grid-cols-1 min-[420px]:grid-cols-2 gap-2">
                                <div>
                                    <label class="block text-[10px] font-medium text-gray-500 mb-1">Bucket incluso</label>
                                    <input type="number" min="0" name="creditos_inclusos_saldo" value="{{ $modalBucket }}" class="w-full text-[13px] py-2.5 px-3 border border-gray-300 rounded">
                                    <p class="text-[10px] text-gray-400 mt-1">Créditos do plano já concedidos (rollover).</p>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-medium text-gray-500 mb-1">Cap auto</label>
                                    <input type="number" min="0" name="limite_consumo_automatico" class="w-full text-[13px] py-2.5 px-3 border border-gray-300 rounded" placeholder="default">
                    ...
                    <div class="px-4 sm:px-5 py-3 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 bg-white" style="flex:0 0 auto">
                        <p class="text-[11px] text-gray-500">Para bloqueio, trial ou impersonação, abra o detalhe completo.</p>
                        <div class="grid grid-cols-1 min-[420px]:grid-cols-2 sm:flex sm:flex-wrap sm:justify-end gap-2 w-full sm:w-auto">
                            <a href="{{ route('app.admin.usuarios.show', $u->id) }}" data-link class="admin-action inline-flex items-center justify-center px-3 py-2 rounded text-[12px] font-semibold text-white hover:opacity-90" style="background-color:#334155">Detalhe completo</a>
                            <a href="{{ route('app.admin.usuarios.edit', $u->id) }}" data-link class="admin-action inline-flex items-center justify-center px-3 py-2 rounded border border-gray-300 text-[12px] font-semibold text-gray-700 bg-white hover:bg-gray-50">Editar cadastro</a>
                        </div>
                    </div>
