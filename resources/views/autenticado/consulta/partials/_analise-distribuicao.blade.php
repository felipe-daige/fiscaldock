{{-- Barra de distribuição (regular/atenção/indeterminado/falha/neutro) de uma fonte da
     Análise da Consulta. Espera $f (com total/regular/atencao/indeterminado/falha/neutro). --}}
...
<div class="flex h-2.5 w-full rounded-full overflow-hidden" style="background-color: #f3f4f6">
    ...
    @if((int) ($f['indeterminado'] ?? 0) > 0)<div style="width: {{ round(($f['indeterminado'] / $tot) * 100, 2) }}%; background-color: #b45309" title="Indeterminado: {{ (int) $f['indeterminado'] }}"></div>@endif
    @if((int) ($f['falha'] ?? 0) > 0)<div style="width: {{ round(($f['falha'] / $tot) * 100, 2) }}%; background-color: #b45309" title="Falha de consulta: {{ (int) $f['falha'] }}"></div>@endif
