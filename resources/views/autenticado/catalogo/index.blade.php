            ...
            <form method="GET" action="/app/catalogo" data-mobile-filters>
                ...
                <div class="relative" data-pop>
                    ...
                    <div data-pop-panel class="hidden absolute z-30 left-0 mt-1 w-80 max-w-[calc(100vw-2.5rem)] bg-white border border-gray-300 rounded shadow-lg">
                        ...
                        <div id="cfopBox" class="max-h-[240px] overflow-y-auto divide-y divide-gray-100">
                            ...
                            @empty
                                <span class="block px-2.5 py-2 text-[11px] text-gray-400">Sem movimentação no filtro.</span>
                            @endforelse
                        </div>
                        @if(count($cfopOpcoes))
                ...
                <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-200">
                    ...
                    @if(array_filter($filtros))
                        <a href="/app/catalogo" data-link class="filtro-acao bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 rounded text-sm font-medium px-4 py-2">Limpar</a>
                    @endif
                </div>
            </form>
        </div>
...
// Filtros CFOP/CST: busca incremental, marcar/limpar visíveis e contador. SPA-safe (sem listeners globais).
...
(function () {
    if (window._cleanupFunctions && window._cleanupFunctions.catalogoPops) {
        window._cleanupFunctions.catalogoPops();
    }
    ...
    window._cleanupFunctions.catalogoPops = function () {
