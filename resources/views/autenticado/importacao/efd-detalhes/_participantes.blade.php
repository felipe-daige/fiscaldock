...
<div class="bg-white rounded border border-gray-300" id="participantes-section">
 ...
 <option value="10" {{ request('per_page_participantes', 10) == 10 ? 'selected' : '' }}>10 por pag.</option>
 <option value="25" {{ request('per_page_participantes') == 25 ? 'selected' : '' }}>25 por pag.</option>
 <option value="50" {{ request('per_page_participantes') == 50 ? 'selected' : '' }}>50 por pag.</option>
 <option value="100" {{ request('per_page_participantes') == 100 ? 'selected' : '' }}>100 por pag.</option>
 </select>
 </div>
 <div class="relative">
 ...
 <th class="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Endereco</th>
 <th class="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Inscricao Estadual</th>
 </tr>
 </thead>
 ...
 <td class="px-6 py-4 text-sm text-gray-900 max-w-[280px] truncate" title="{{ $part->razao_social ?: 'Razao social nao informada' }}">{{ $part->razao_social ?: '—' }}</td>
 ...
 <p class="text-sm font-medium text-gray-700">Importacao em andamento</p>
 <p class="text-xs text-gray-500 mt-1">Os participantes aparecerão aqui quando o processamento for concluido.</p>
 @elseif($importacao->status === 'erro')
 <svg class="w-10 h-10 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
 </svg>
 <p class="text-sm font-medium text-gray-700">Nenhum participante extraido</p>
 <p class="text-xs text-gray-500 mt-1">A importacao terminou com erro. Nenhum participante foi extraido.</p>
 @else
 <svg class="w-10 h-10 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <p class="text-xs text-gray-500 mt-1">Esta importacao nao gerou participantes.</p>
 @endif
 </div>
 @endif
</div>
