...
class RafLote extends Model
{
    use HasFactory;

    protected $table = 'raf_lotes';
    ...
    {
        return $this->belongsToMany(Participante::class, 'raf_lote_participantes', 'raf_lote_id', 'participante_id')
    ...
    {
        return $this->hasMany(RafLoteResultado::class, 'raf_lote_id');
    }
    ...
    {
        return $this->resultados()->where('status', RafLoteResultado::STATUS_SUCESSO)->exists();
    }
    ...
    public function getContadoresResultados(): array
    {
        return [
            ...
            'sucesso' => $this->resultados()->where('status', RafLoteResultado::STATUS_SUCESSO)->count(),
            'erro' => $this->resultados()->whereIn('status', [RafLoteResultado::STATUS_ERRO, RafLoteResultado::STATUS_TIMEOUT])->count(),
            'pendente' => $this->resultados()->where('status', RafLoteResultado::STATUS_PENDENTE)->count(),
        ];
    }
