...
class Participante extends Model
{
    ...
    /**
     * Exclui qualquer documento que já pertença à carteira de Clientes do usuário.
     *
     * Cliente e Participante são identidades mutuamente exclusivas. O NOT EXISTS também
     * protege leituras durante rollout/backfill de bases legadas.
     */
    public function scopeExcluindoClientes($query)
    {
        $query->whereNotExists(function ($sub) {
            $sub->selectRaw('1')
                ...
                ->whereRaw(
                    "regexp_replace(coalesce(clientes.documento, ''), '[^0-9]', '', 'g')"
                    ." = regexp_replace(coalesce(participantes.documento, ''), '[^0-9]', '', 'g')"
                );
        });
