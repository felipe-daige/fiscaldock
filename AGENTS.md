...
- CND Federal InfoSimples `611` must be mapped to `cnd_federal.status = INDETERMINADO`, not irregular. It means the official source could not emit the certificate online because Receita Federal/origin data is insufficient; preserve the original `errors[]` message.
